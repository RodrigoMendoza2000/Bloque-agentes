...
class Car(Agent):
    ...
    Attributes:
        ...
        direction: Randomly chosen direction chosen 
        from one of eight directions
    """
    ...
    def __init__(self, unique_id, model):
        ...
        self.roundabout_coordinates = None
        self.path = None
    ...
    def move(self):
        ...
        if self.must_be_assigned_destination:
            ...
            safety_counter = 0
            while self.has_next_step_agent(random_destination,
                                           "Car") and safety_counter < 100:
            ...
            if safety_counter < 100:
                ...
                self.from_destination = random_destination
                return
            # If there is no available destination in 100 iterations, 
        ...
        if self.final_destination is None:
            ...
            # Apply the Dijkstra algorithm to find the shortest path to the 
            # final destination
            dijkstra_city = DijkstraCoordinate(
                graph=simCity, start_coordinate=self.from_destination)
            dijkstra_city.dijkstra()
            self.path = dijkstra_city.shortest_path_coordinates(
        ...
        # If the car has arrived to the final destination, remove the agent 
        ...
        self.roundabout_coordinates = [(12, 9), (12, 8), (13, 13), (14, 13),
                                       (18, 12), (18, 11), (16, 7), (17, 7)]

        # If it is on a position before entering the roundabout, 
        # check diagonally if there is any car,
        ...
        # Go to position depending on the position of the node and 
        # self direction
        if self.pos[0] == self.path[0][0] and self.pos[1] - self.path[0][1] \
        ...
        elif self.pos[0] == self.path[0][0] and self.pos[1] - self.path[0][1] \
                < 0:
            ...
            if self.is_position_valid_for_parking(next_step):
                self.model.grid.move_agent(self, next_step)
        elif self.pos[0] - self.path[0][0] > 0 and self.pos[1] == \
                self.path[0][1]:
        ...
        # To make the turning variable True to allow a smoother
        # transition in Unity
        if len(self.path) > 1:
            if self.pos[0] == self.path[1][0] and self.pos[1] - \
                    self.path[1][1] > 0:
                self.set_turn_conditional("Down")
            elif self.pos[0] == self.path[1][0] and self.pos[1] - \
                    self.path[1][1] < 0:
                self.set_turn_conditional("Up")
            elif self.pos[0] - self.path[1][0] > 0 and self.pos[1] == \
                    self.path[1][1]:
                self.set_turn_conditional("Left")
            elif self.pos[0] - self.path[1][0] < 0 and self.pos[1] == \
                    self.path[1][1]:
    ...
    def is_position_valid_for_parking(self, position):
        ...
        is a road or is a traffic light on green or it is a parking spot 
        (destination)


        Args:
            position (tuple(int, int)): The coordinates of the position 
            to check
        ...
        if not self.model.grid.out_of_bounds(position):
            ...
            if len(cell_content) == 1 and (
                    isinstance(cell_content[0], Road) or
                    isinstance(cell_content[0], Traffic_Light)) or \
                    isinstance(cell_content[0], Destination):
    ...
    def has_next_step_agent(self, next_step, agent_type):
        ...
        Args:
            next_step (tuple(int, int)): the coordinates of where the agent 
            will go next
            agent_type (string): The name of the agent type to check for

        Returns:
            bool: returns True if the next step has an agent of the type 
    ...
    def set_turn_conditional(self, next_direction):
        ...
        Args:
            next_direction (string): the direction that will follow the 
...
class Traffic_Light(Agent):
    ...
    def step(self):
        ...
        # in that street
        self.total_cars = self.current_cars + \
    ...
    def get_direction_of_cars(self):
        ...
        for agent in self.model.grid.iter_neighbors(self.pos, moore=False):
            if isinstance(agent, Road):
                if (partner_direction in ("Up", "Down")) and (
                        agent.direction in ("Left", "Right")):
                    return agent.direction
                elif (partner_direction in ("Left", "Right")) and (
...
class Bus(Car):
    ...
    def step(self):
        ...
        # If the bus is in a bus destination and waiting, wait 3 steps 
        # then move.
        if self.stopping:
            self.waiting_time += 1
        ...
        # Follow the path of the bus until it reaches the next element 
        # in the list 
        ...
        if self.pos[0] == self.path[0][0] and self.pos[1] - self.path[0][1] \
            ...
            if self.is_position_valid_for_parking(next_step):
                self.model.grid.move_agent(self, next_step)
        elif self.pos[0] == self.path[0][0] and self.pos[1] - \
                self.path[0][1] < 0:
            self.set_turn_conditional("Up")
            self.direction = "Up"
            next_step = self.next_step_based_on_direction_self()
        ...
        # If the bus is in a bus destination, stop for 3 steps 
        # in the bus destination
        for agent in self.model.grid.iter_neighbors(self.pos, moore=False):
            if isinstance(agent, Busdestination) and self.stopping is False:
                self.stopping = True


class Person(Agent):
    """
    Person agent that will move around the sidewalk, and will be able to get 
    into a bus
    """
    ...
    def step(self):
        ...
        # If the person passes through the BusDestination, it will have a 10% 
        # chance of waiting for a bus
        if not self.in_bus and "Busdestination" in self.get_cell_class_names(
                self.pos):
    ...
    def is_valid_position(self, position):
        ...
        if not self.model.grid.out_of_bounds(position):
            ...
            if "Road" in cell_content or "Traffic_Light" in cell_content or \
                    "Obstacle" in cell_content:
                return False
            else:
                return True
    ...
    def next_step_based_on_direction(self, direction):
        ...
        Args:
            direction (string): The direction to move into 
            (Up, Down, Left, Right)
    ...
    def next_direction(self, direction, initial_direction):
        """
        Know where to move next depending on the direction and the 
        initial direction

        Args:
            direction (string): The current direction of the agent
            initial_direction (string): The direction assigned at the 
            initialization of the agent

        Returns:
            string: The next direction to move into and be assigned 
            to the agent
        """
        if direction == "Up":
